...
public class DBUtil {
    ...
    public static final String USER_CRAWL_STATE = UserCrawlState.class
            .getSimpleName();
    public static final String REPOST_CRAWL_STATE = RepostCrawlState.class
            .getSimpleName();
    public static final String CRAWL_STATS = "crawlstates";

    public static String genStatusTable() {
        StringBuffer buf = new StringBuffer(
                "create table IF NOT EXISTS " + MICROBLOG_TABLE
                        + "(omid text,uid text, uname text,");
    ...
    public static String genTableForClass(Class tableClass, String priKey) {
        StringBuffer buf = new StringBuffer(
                "create table IF NOT EXISTS " + tableClass.getSimpleName()
                        + "(");
    ...
    public static String genObjFields(Class sclass) {
        ...
        for (Field field : fields) {
            if (!Modifier.isStatic(field.getModifiers())
                    && field.getType() != User.class
                    && field.getType() != Status.class && !field.getName()
                    .startsWith("_")) {
    ...
    public static abstract class PrepareStatementGen
    ...
    public static class PrepareStatementBounder implements ObjectInspector {
        ...
        public void process(Object obj, Field field) {
            try {
                if (!Modifier.isStatic(field.getModifiers()) && !field.getName()
                        .startsWith("_")) {
                    field.setAccessible(true);
                    if (field.getType() == String.class) {
                        bound.setString(field.getName(),
                        ...
                        if (value == null) {
                            ...
                            bound.setLong(field.getName(),
                                    ((Date) field.get(obj)).getTime());
                        }
                    } else {
     ...
     * @author xiafan
     */
    public static class StatusFieldValueInSpector
            extends PrepareStatementBounder {
    ...
    public static String genPrepareStatement(Class sclass,
            PrepareStatementGen inspect) {
        ...
        return "INSERT INTO " + sclass.getSimpleName() + String
                .format("(%s) VALUES(%s)", fieldBuf.toString(),
                        valBuf.toString());
    }
    ...
    public static void main(String[] args) throws weibo4j.model.WeiboException {
        System.out.println(
                genPrepareStatement(Status.class, new StatusFieldInSpector()));
        System.out.println(
                genPrepareStatement(User.class, new StatusFieldInSpector()));
    }
}
