...
public class RTTreeConstructor {
		...
		for (Tweet t : tweets) {
			...
			if (matcher.find()) {
				...
				while (!stack.isEmpty()) {
					...
					curContent = String.format("%s%s%s:%s", pair.getValue(), RT_TAG, preName, curContent);
					preName = pair.getKey();
					Tweet internNode = null;
					if (!stack.isEmpty()) {
						internNode = new Tweet("", curContent, pair.getKey(), "", -1, oT.getMid());
						UUID uid = UUID.randomUUID();
						internNode.setMid(Long.toString(uid.getLeastSignificantBits() & uid.getMostSignificantBits()));
						// internNode.setMid(String.format("-%s%s", oT.getMid(),
						// midGen++));
						internNode.setUid("-" + Integer.toString(pair.getKey().hashCode()));
					} else {
						internNode = t;
				...
				t.setContent(t.getContent().trim() + RT_TAG + oT.getUname() + ":" + oT.getContent());
				path.add(t);
			}
			tree.addPath(path);
		}
		...
		for (Tweet t : tweets) {
			...
			if (matcher.find()) {
				...
				while (!stack.isEmpty()) {
					...
					if (!stack.isEmpty()) {
						internNode = new Tweet("", curContent, pair.getKey(), "", -1, oT.getMid());
						internNode.setMid(String.format("-%s%s", midBase, midGen++));
						internNode.setUid("-" + Integer.toString(pair.getKey().hashCode()));
					} else {
						internNode = t;
						t.setContent(curContent);
	...
	public static TrieTree<Tweet> parse_pre(Tweet oT, List<Tweet> tweets) {
		...
		for (Tweet t : tweets) {
			ArrayList<Tweet> path = new ArrayList<Tweet>();
			path.add(oT);
			String curContent = new String(RT_TAG + oT.getUname() + ":" + oT.getContent());
			if (t.getContent().contains(RT_TAG)) {
				...
				do {
					...
					if (idx >= 0) {
						...
						curContent = String.format("%s%s%s", RT_TAG, node, curContent);
					} else {
					...
					if (idx > 0 && deIdx > 0) {
						...
						internNode = new Tweet("", curContent, uname, "", -1, oT.getMid());
						UUID uuid = UUID.randomUUID();
						// internNode.setMid(String.format("-%s%s", oT.getMid(),
						// midGen++));
						internNode.setMid(
								Long.toString((uuid.getMostSignificantBits() & uuid.getLeastSignificantBits())));
						internNode.setUid("-" + Integer.toString(uname.hashCode()));
					} else {
						internNode = t;
						t.setContent(curContent);
					}
	...
	public static class ZipfTsGen implements ITimeStampGenerator {
		...
		public long chooseTs(long start, long end) {
			...
			long ts = -1;
			double alpha = (1 - theta) / (Math.pow(end, 1 - theta) - Math.pow(start, 1 - theta));
			float z = rand.nextFloat();
			ts = (long) Math.pow((1 - theta) * z / alpha + Math.pow(start, 1 - theta), 1 / (1 - theta));
			return ts;
		}
	}
	 ...
	 * @param curNode
	 * @param upBound
	 *            the timestamp of the parent node plus one
	 */
	...
	public static void main(String[] args) throws IOException {
		for (int i = 0; i < 1000; i++) {
			UUID uid = UUID.randomUUID();
			System.out.println(uid.getLeastSignificantBits() & uid.getMostSignificantBits());
		}
		...
		TrieTree<Tweet> tree = RTTreeConstructor.parseTwitter(ot, Arrays.asList(t));
	...
	private static void tweetTest() throws IOException {
		BufferedReader reader = new BufferedReader(new InputStreamReader(
				new DataInputStream(new BufferedInputStream(
						new FileInputStream("/Users/xiafan/Documents/dataset/expr/twitter/orgin/part0.txt"))),
				"utf-8"));
		String line = null;
		Tweet ot = null;
	...
	private static void statusTest() throws IOException {
		BufferedReader reader = new BufferedReader(new InputStreamReader(new DataInputStream(
				new BufferedInputStream(new FileInputStream("/Users/xiafan/Documents/dataset/microblog/城管.txt"))),
				"utf-8"));
		String line = null;
		Tweet ot = null;
		ArrayList<Tweet> tweets = new ArrayList<Tweet>();
