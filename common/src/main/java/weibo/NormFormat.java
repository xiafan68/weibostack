...
public class NormFormat {
     ...
     * @throws Exception
     */
    public static TweetWithID transToFullTweetOfSimple(String line)
            throws Exception {
        ...
        if (fields.length >= 12) {
            ...
            throw new Exception(
                    "omid is " + fields[1] + "more fields are obtained "
                            + fields.length);
        }
        return ret;
    }
     ...
     * @author xiafan
     */
    public static Pair<Long, Tweet> transToTweetOfSimple(String line)
            throws Exception {
        ...
        if (fields.length >= 5) {
            ...
            ret.setValue(new Tweet(fields[4], fields[5], fields[7], fields[6],
                    Long.parseLong(fields[2]), fields[1]));
        } else {
            throw new Exception(
                    "omid is " + fields[1] + "more fields are obtained "
                            + fields.length);
     ...
     * @author xiafan
     */
    public static Pair<Tweet, Tweet> transToTweets(String line)
            throws Exception {
        ...
        if (idx < 0) {
            ...
            String[] fields = line.substring(idx, line.length()).trim()
                    .split(SEP_REGRX);

            if (fields.length >= 11) {
                ret = new Pair<Tweet, Tweet>();

                ret.setValue(new Tweet(oMid, fields[7], fields[10], fields[9],
                        Long.parseLong(fields[6]), "-1"));
                ret.setKey(new Tweet(fields[1], fields[2], fields[5], fields[4],
                        Long.parseLong(fields[0]), ret.getValue().getMid()));
                ...
                throw new Exception(
                        "omid is " + oMid + "more fields are obtained "
                                + fields.length);
            }
        }
        return ret;
    ...
    public static Tweet parseTweetFromFields(List<String> fields) {
        ...
        try {
            if (fields.size() >= 7) {
                t = new Tweet(fields.get(1), fields.get(3), fields.get(6),
                        fields.get(5), Long.parseLong(fields.get(2)), "-1");
    ...
    public static void main(String[] args) throws Exception {
        ...
        BufferedReader reader = new BufferedReader(new InputStreamReader(
                new FileInputStream(
                        "/Users/xiafan/Documents/dataset/microblog/sample.txt"),
                "utf-8"));

        DataOutputStream dos = new DataOutputStream(
                new FileOutputStream("./data/sample_norm.txt"));
        String line = null;
        int count = 0;
