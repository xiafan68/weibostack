...
public class IKTextShingle extends MicroblogShingle {
	IKSegmenter segmenter = new IKSegmenter(new StringReader(""), true);
	public boolean preserveUName = false;

	public IKTextShingle(StopWordFilter filter) {
		super(filter);
	}
	...
	public static void main(String[] args) throws IOException {
		IKTextShingle shingle = new IKTextShingle(null);
		...
		for (String tweet : tweets) {
			System.out.println(String.format("%s \n --> %s\n -->", tweet, shingle.shingling(tweet, false).toString()));
		}
		shingle.shingling("//@ab:Ψήφο στην κομμουνιστική αριστερά που πρέπει νααλλάξει http://bit.ly/Cyq6h", false);

		String[] puncTests = new String[] { "@haha:", "@:", "@asdf :", " @ haha:", "@ haha : sdf", "//@ haha : sdf",
				"// @ haha : sdf", "// @ haha : //sdf", "// @ @haha: : //sdf" };
		for (String punc : puncTests) {
			// System.out.println(String.format("%s --> %s", punc,
			// shingle.postProcess(punc)));
		}
		...
		System.out.println(shingle.shingling("苹果手机# iphone4 #ss", false));
		System.out.println(shingle.shingling("#^&*", false));
	}
}
